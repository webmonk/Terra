...
pub struct ShortcutModifiers { pub ctrl: bool, pub shift: bool, pub alt: bool, pub super_key: bool }
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ShortcutChord { pub key: KeyCode, pub modifiers: ShortcutModifiers }
impl ShortcutChord {
    pub const fn new(key: KeyCode, modifiers: ShortcutModifiers) -> Self { Self { key, modifiers } }
    pub fn is_unmodified_text_key(self) -> bool {
        self.modifiers == ShortcutModifiers::default() && matches!(self.key,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BindingVisibility { Displayed, LegacyAlias }
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ShortcutBinding { pub chord: ShortcutChord, pub visibility: BindingVisibility }
impl ShortcutBinding {
    pub const fn displayed(key: KeyCode, modifiers: ShortcutModifiers) -> Self { Self { chord: ShortcutChord::new(key, modifiers), visibility: BindingVisibility::Displayed } }
    pub const fn alias(key: KeyCode, modifiers: ShortcutModifiers) -> Self { Self { chord: ShortcutChord::new(key, modifiers), visibility: BindingVisibility::LegacyAlias } }
}
const NONE: ShortcutModifiers = ShortcutModifiers { ctrl: false, shift: false, alt: false, super_key: false };
const CTRL: ShortcutModifiers = ShortcutModifiers { ctrl: true, ..NONE };
const SHIFT: ShortcutModifiers = ShortcutModifiers { shift: true, ..NONE };
const CTRL_SHIFT: ShortcutModifiers = ShortcutModifiers { ctrl: true, shift: true, ..NONE };
...
const QUICK_ADD: &[ShortcutBinding] = &[ShortcutBinding::displayed(KeyCode::KeyL, CTRL), ShortcutBinding::displayed(KeyCode::Insert, NONE)];
const PALETTE: &[ShortcutBinding] = &[ShortcutBinding::displayed(KeyCode::KeyP, CTRL)];
const UNDO: &[ShortcutBinding] = &[ShortcutBinding::displayed(KeyCode::KeyZ, CTRL), ShortcutBinding::alias(KeyCode::KeyZ, NONE)];
const REDO: &[ShortcutBinding] = &[ShortcutBinding::displayed(KeyCode::KeyY, CTRL), ShortcutBinding::alias(KeyCode::KeyZ, CTRL_SHIFT), ShortcutBinding::alias(KeyCode::KeyZ, SHIFT), ShortcutBinding::alias(KeyCode::KeyY, NONE)];
...
fn digit_bindings(digit: u8) -> &'static [ShortcutBinding] { match digit { 1 => D1, 2 => D2, 3 => D3, 4 => D4, 5 => D5, 6 => D6, 7 => D7, 8 => D8, _ => &[] } }
...
pub fn resolve_shortcut(chord: ShortcutChord) -> Option<&'static str> { commands().into_iter().find_map(|c| c.bindings.iter().any(|b| b.chord == chord).then_some(c.id)) }
pub fn resolve_shortcut_for_input(chord: ShortcutChord, text_input: bool) -> Option<&'static str> { if text_input && chord.is_unmodified_text_key() { None } else { resolve_shortcut(chord) } }
pub fn format_shortcuts(bindings: &[ShortcutBinding]) -> String { bindings.iter().filter(|b| b.visibility == BindingVisibility::Displayed).map(|b| format_chord(b.chord)).collect::<Vec<_>>().join(" / ") }
fn format_chord(chord: ShortcutChord) -> String {
    let mut p = Vec::new(); if chord.modifiers.ctrl { p.push("Ctrl"); } if chord.modifiers.alt { p.push("Alt"); } if chord.modifiers.shift { p.push("Shift"); } if chord.modifiers.super_key { p.push("Super"); }
    p.push(match chord.key { KeyCode::Digit1=>"1",KeyCode::Digit2=>"2",KeyCode::Digit3=>"3",KeyCode::Digit4=>"4",KeyCode::Digit5=>"5",KeyCode::Digit6=>"6",KeyCode::Digit7=>"7",KeyCode::Digit8=>"8",KeyCode::KeyF=>"F",KeyCode::KeyL=>"L",KeyCode::KeyN=>"N",KeyCode::KeyO=>"O",KeyCode::KeyP=>"P",KeyCode::KeyS=>"S",KeyCode::KeyW=>"W",KeyCode::KeyY=>"Y",KeyCode::KeyZ=>"Z",KeyCode::Insert=>"Insert",_=>"Unknown" }); p.join("+")
}
...
pub fn commands() -> Vec<CommandDef> {
    ...
    for definition in all_workspace_definitions() {
        ...
        out.push(CommandDef {
            ...
            name,
            category: Mode,
            bindings: definition.id.digit_shortcut().map(digit_bindings).unwrap_or(&[]),
...
mod tests {
    ...
    fn command_ids_and_executable_shortcuts_are_unique() {
        ...
        for command in &commands {
            ...
            for binding in command.bindings { assert!(shortcuts.insert(binding.chord), "duplicate executable shortcut: {:?}", binding.chord); }
        }
    }
    ...
    fn required_shortcuts_are_executable_and_display_is_derived() {
        ...
        assert_eq!(resolve_shortcut(c(KeyCode::KeyF, NONE)), Some(CommandId::FRAME_TERRAIN));
        assert_eq!(resolve_shortcut(c(KeyCode::KeyP, CTRL)), Some(CommandId::OPEN_COMMAND_PALETTE));
        assert_eq!(resolve_shortcut(c(KeyCode::KeyL, CTRL)), Some(CommandId::OPEN_QUICK_ADD));
        assert_eq!(resolve_shortcut(c(KeyCode::Insert, NONE)), Some(CommandId::OPEN_QUICK_ADD));
        assert_eq!(resolve_shortcut(c(KeyCode::KeyS, CTRL)), Some(CommandId::SAVE));
        assert_eq!(resolve_shortcut(c(KeyCode::KeyS, CTRL_SHIFT)), Some(CommandId::SAVE_AS));
        assert_eq!(resolve_shortcut(c(KeyCode::KeyZ, CTRL)), Some(CommandId::UNDO));
        assert_eq!(resolve_shortcut(c(KeyCode::KeyZ, NONE)), Some(CommandId::UNDO));
        for redo in [c(KeyCode::KeyY, CTRL), c(KeyCode::KeyZ, CTRL_SHIFT), c(KeyCode::KeyZ, SHIFT), c(KeyCode::KeyY, NONE)] { assert_eq!(resolve_shortcut(redo), Some(CommandId::REDO)); }
        for command in commands() { for binding in command.bindings.iter().filter(|b| b.visibility == BindingVisibility::Displayed) { assert_eq!(resolve_shortcut(binding.chord), Some(command.id)); } assert!(!format_shortcuts(command.bindings).contains("Unknown")); }
        let quick = commands().into_iter().find(|c| c.id == CommandId::OPEN_QUICK_ADD).unwrap();
        assert_eq!(format_shortcuts(quick.bindings), "Ctrl+L / Insert");
    }

    #[test]
    fn modifiers_and_text_input_are_respected() {
        let ctrl_alt = ShortcutModifiers { ctrl: true, alt: true, ..NONE };
        assert_eq!(resolve_shortcut(ShortcutChord::new(KeyCode::KeyS, ctrl_alt)), None);
        assert_eq!(resolve_shortcut(ShortcutChord::new(KeyCode::Digit1, CTRL)), None);
        assert_eq!(resolve_shortcut_for_input(ShortcutChord::new(KeyCode::KeyF, NONE), true), None);
        assert_eq!(resolve_shortcut_for_input(ShortcutChord::new(KeyCode::Digit1, NONE), true), None);
        assert_eq!(resolve_shortcut_for_input(ShortcutChord::new(KeyCode::KeyP, CTRL), true), Some(CommandId::OPEN_COMMAND_PALETTE));
        assert_eq!(resolve_shortcut_for_input(ShortcutChord::new(KeyCode::Insert, NONE), true), Some(CommandId::OPEN_QUICK_ADD));
    }
