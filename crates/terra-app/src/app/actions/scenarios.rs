    ...
    let result = match action {
                PanelAction::AddSimulationScenario { name } => {
                    ...
                    app.session.push_scenario_command(
                        terra_core::simulation_scenario::SimulationScenarioCommand::Add {
                            scenario,
                            index,
                        },
                    );
                ...
                PanelAction::AddContinentalHydrologyPreset => {
                    let scenario = terra_core::simulation_scenario::SimulationScenario::continental_hydrology_preset();
                    let index = app.session.document.simulation_scenarios.scenarios.len();
                    app.session.push_scenario_command(
                        terra_core::simulation_scenario::SimulationScenarioCommand::Add {
                            scenario,
                ...
                PanelAction::ResetSimulationScenario(id) => {
                    if let Some(before) =
                        app.session.document.simulation_scenarios.get(id).cloned()
                    {
                ...
                PanelAction::ApplyScenarioOutputs { id, snapshot } => {
                    if let Some(scen) = app.session.document.simulation_scenarios.get_mut(id) {
                        ...
                        app.ui_state.status =
                            format!("Applied {} scenario output field(s)", fields.len());
                        let preview = app.session.document.preview_eval_stack();
                        app.scheduler.evaluator.mark_dirty_from_eval_stage(
                            &preview,
                ...
                PanelAction::SetMatterSourcePaint {
                    ...
                    if let Some(before) = app                        .session
                        .document
                        .simulation_scenarios
                        .get(scenario)
                        .cloned()
                ...
                PanelAction::SetMatterApplySelected {
                        ...
                        .cloned()
                    {
                        ...
                        if let Some(m) = after.matter.get(matter_index).cloned() {
                            terra_core::matter_sim::sync_scenario_outputs_from_matter(
                                &mut after, &m,
                            );
                        }
                        app.session.push_scenario_command(
