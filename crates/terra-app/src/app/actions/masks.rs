...
pub(crate) fn try_apply(
    ...
    let result = match action {
                ...
                PanelAction::UpdateMaskAsset(asset) => {
                    if let Some(existing) = app                        .session
                        .document
                        .masks
                        .iter_mut()
                ...
                PanelAction::UpdateMaskBinding {
                            ...
                            })
                    {
                        ...
                        ctx.dirty_from = Some(layer);
                    } else if let Some(binding) = app                        .session
                        .document
                        .stack
                        .find_group_mut(layer)
                        .and_then(|group| {
                            group.masks.iter_mut().find(|entry| entry.mask.id == mask)
                        })
                    {
                        binding.mask.strength = strength;
                        binding.mask.invert = invert;
                ...
                PanelAction::PaintMaskStamp {
                    ...
                    if app.mask_paint_stroke_before.is_none() {
                        if let Some(asset) =
                            app.session.document.masks.iter().find(|a| a.id == mask_id)
                        {
                            if let Some(paint) = asset.paint.as_ref() {
                                app.mask_paint_stroke_before = Some((
                                    mask_id,
                                    paint.samples.clone(),
                                    paint.width,
                                    paint.height,
                                ...
                                app.mask_paint_stroke_before =
                                    Some((mask_id, Vec::new(), 512, 512));
                            }
                        }
                    }
                    if let Some(asset) = app                        .session
                        .document
                        .masks
                        .iter_mut()
                ...
                PanelAction::PaintSculptStamp {
                    ...
                    if let Some(target) = app.session.document.stack.find_mut(layer) {
                            ...
                            &mut target.kind
                        {
                            ...
                            ctx.dirty_from = Some(layer);
                            app.session.document.selected = Some(layer);
                        } else if let terra_core::layer::LayerKind::SculptBase(params) =
                            &mut target.kind
                        {
                ...
                PanelAction::AddDistNode { target, kind } => {
                    ...
                    if let Some(g) = app.session.document.stack.find_group_mut(target) {
                        ...
                        TerraApp::mark_biome_placement_custom_for_group(
                            &mut app.session.document,
                            target,
                        );
                        app.mark_all_layers_dirty();
                        app.request_rebuild();
