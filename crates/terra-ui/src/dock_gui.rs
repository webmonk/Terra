...
use terra_gui::style::{self, FONT_SCALE, PAD, STATUS_STRIP_H};
...
pub fn draw_bottom_dock(
    ...
    let res = if ui_state.profile.tex_w > 0 {
        format!(
            "{}x{}",
            ui_state.profile.tex_w, ui_state.profile.tex_h
        )
    } else {
    ...
    let status_w = DrawList::text_width(&status_text, FONT_SCALE * 0.88);
    ...
    if show_progress {
        ...
        ui.label_centered_in_rect(chip, "Cancel", style::TEXT, FONT_SCALE * 0.85);
        rx -= cancel_w + 8.0;
    ...
    let left = format!(
        "Vertices {:.2}M   Triangles {:.2}M   Resolution {res}   Preview Quality: {quality}   {backend}   Build Time: {build_ms:.0} ms",
        verts, tris
    );
    let left = DrawList::truncate_to_width(&left, FONT_SCALE * 0.88, left_max_w);
    ui.label_at(
        dock.min_x + PAD,
        y,
        &left,
